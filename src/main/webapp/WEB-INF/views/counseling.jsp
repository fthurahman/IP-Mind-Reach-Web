...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .avatar.bot { background: rgba(202, 219, 183, 0.4); }
    .avatar.user { background: rgba(216, 167, 158, 0.4); }
    ...
    .bubble.bot { background: #fff; color: var(--text); }
    .bubble.user { background: #d9e4cc; color: #2a2928; }
    ...
    .pill:hover { background: rgba(202, 219, 183, 0.3); }
...
<body>
  ...
  <script>
    ...
    async function fetchBotReply(userText) {
      ...
      console.log("chatbot/message status:", res.status);
      console.log("chatbot/message content-type:", contentType);
      console.log("chatbot/message raw:", raw);

      // If server returned HTML/view instead of JSON
      if (contentType.indexOf("application/json") === -1) {
        const shortRaw = raw.replace(/\s+/g, " ").slice(0, 180);
        return "Backend returned non-JSON (check server). Response: " + shortRaw;
      }
      ...
      try {
        ...
        const shortRaw = raw.replace(/\s+/g, " ").slice(0, 180);
        return "JSON parse failed. Response: " + shortRaw;
      }
    }
    ...
    async function sendToAI(userText) {
      ...
      try {
        ...
        // Optional action pills after reply
